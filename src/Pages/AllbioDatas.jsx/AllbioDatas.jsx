  ...
  useEffect(() => {
    fetch(
      `https://b8a12-server-side-six.vercel.app/allbiodatas?sort=${
        ...
        Male ? "Male" : "Female"
      }&division=${selectedDivision}&page=${page}`
    )
      .then((res) => res.json())
      .then((data) => setAllbiodatas(data.result));
  }, [asc, Male, selectedDivision, page]);
  return (
    <div>
      <div className="flex">
      ...
      <div className="flex justify-center items-center gap-2">
        <Button className="bg-[#e92f83]" onClick={() => setPage(page - 1)}>
          Prevous Page
        </Button>
        <Button className="bg-[#e92f83]" onClick={() => setPage(page + 1)}>
          {" "}
